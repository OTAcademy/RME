...
static std::string getBrushTypeName(Brush* brush) {
	if (!brush) return "unknown";

	if (brush->isRaw()) return "raw";
	if (brush->isDoodad()) return "doodad";
	if (brush->isGround()) return "ground";
	if (brush->isWall()) return "wall";
	if (brush->isWallDecoration()) return "wall_decoration";
	if (brush->isTable()) return "table";
	if (brush->isCarpet()) return "carpet";
	if (brush->isDoor()) return "door";
	if (brush->isOptionalBorder()) return "optional_border";
	if (brush->isCreature()) return "creature";
	if (brush->isSpawn()) return "spawn";
	if (brush->isHouse()) return "house";
	if (brush->isHouseExit()) return "house_exit";
	if (brush->isWaypoint()) return "waypoint";
	if (brush->isFlag()) return "flag";
	if (brush->isEraser()) return "eraser";
	if (brush->isTerrain()) return "terrain";
...
void registerBrush(sol::state& lua) {
	...
	lua.new_usertype<Brush>("Brush",
		...
		// Type checks
		"isRaw", sol::property([](Brush* b) { return b && b->isRaw(); }),
		"isDoodad", sol::property([](Brush* b) { return b && b->isDoodad(); }),
		"isTerrain", sol::property([](Brush* b) { return b && b->isTerrain(); }),
		"isGround", sol::property([](Brush* b) { return b && b->isGround(); }),
		"isWall", sol::property([](Brush* b) { return b && b->isWall(); }),
		"isTable", sol::property([](Brush* b) { return b && b->isTable(); }),
		"isCarpet", sol::property([](Brush* b) { return b && b->isCarpet(); }),
		"isDoor", sol::property([](Brush* b) { return b && b->isDoor(); }),
		"isCreature", sol::property([](Brush* b) { return b && b->isCreature(); }),
		"isSpawn", sol::property([](Brush* b) { return b && b->isSpawn(); }),
		"isHouse", sol::property([](Brush* b) { return b && b->isHouse(); }),
		"isEraser", sol::property([](Brush* b) { return b && b->isEraser(); }),

		// String representation
		sol::meta_function::to_string, [](Brush* b) -> std::string {
			if (!b) return "Brush(invalid)";
	...
	lua.new_enum("BrushShape",
		"SQUARE", BRUSHSHAPE_SQUARE,
		"CIRCLE", BRUSHSHAPE_CIRCLE
	);
}
