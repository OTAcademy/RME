...
namespace LuaAPI {
	...
	void registerColor(sol::state& lua) {
		...
		lua.new_usertype<LuaColor>("Color",
								   // Constructors
								   sol::constructors<LuaColor(), // Color()
								   ...
								   "red", &LuaColor::red, "green", &LuaColor::green, "blue", &LuaColor::blue, "alpha", &LuaColor::alpha,

								   // Shorthand aliases
								   "r", &LuaColor::red, "g", &LuaColor::green, "b", &LuaColor::blue, "a", &LuaColor::alpha,
								   ...
								   sol::meta_function::to_string, [](const LuaColor& c) {
									   return "Color(" + std::to_string(c.red) + ", " + std::to_string(c.green) + ", " + std::to_string(c.blue) + (c.alpha != 255 ? ", " + std::to_string(c.alpha) : "") + ")";
								   });
