...
void registerAlgo(sol::state& lua) {
	...
	algoTable.set_function("thermalErode", [](sol::table inputHeightmap, sol::optional<sol::table> options, sol::this_state s) -> sol::table {
		...
		if (width <= 2 || height <= 2) return inputHeightmap;
	...
	algoTable.set_function("smooth", [](sol::table inputGrid, sol::optional<sol::table> options, sol::this_state s) -> sol::table {
		...
		if (width <= 2 || height <= 2) return inputGrid;
		...
		for (size_t i = 1; i < rooms.size(); ++i) {
			...
			if (rng() % 2 == 0) {
				...
				for (int x = std::min(x1, x2); x <= std::max(x1, x2); ++x) {
					if (y1 >= 0 && y1 < height && x >= 0 && x < width) grid[y1][x] = 0;
				}
				for (int y = std::min(y1, y2); y <= std::max(y1, y2); ++y) {
					if (y >= 0 && y < height && x2 >= 0 && x2 < width) grid[y][x2] = 0;
				}
			} else {
				// Vertical first
				for (int y = std::min(y1, y2); y <= std::max(y1, y2); ++y) {
					if (y >= 0 && y < height && x1 >= 0 && x1 < width) grid[y][x1] = 0;
				}
				for (int x = std::min(x1, x2); x <= std::max(x1, x2); ++x) {
					if (y2 >= 0 && y2 < height && x >= 0 && x < width) grid[y2][x] = 0;
				}
			}
		}
