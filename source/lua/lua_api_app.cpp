...
static void updateTileMetadata(Editor* editor, Tile* tile, bool adding) {
	...
	if (adding) {
		if (tile->spawn) map->addSpawn(tile);
		if (tile->getHouseID()) {
			House* h = map->houses.getHouse(tile->getHouseID());
			if (h) h->addTile(tile);
		}
	} else {
		if (tile->spawn) map->removeSpawn(tile);
		if (tile->getHouseID()) {
			House* h = map->houses.getHouse(tile->getHouseID());
			if (h) h->removeTile(tile);
		}
...
public:
	...
	LuaTransaction() : active(false), editor(nullptr), batch(nullptr), action(nullptr) {}
...
void registerApp(sol::state& lua) {
	...
	mt[sol::meta_function::index] = [](sol::this_state ts, sol::table self, std::string key) -> sol::object {
		...
		if (key == "map") {
			...
			if (b) return sol::make_object(lua, b);
			return sol::nil;
		}
	...
	mt[sol::meta_function::new_index] = [](sol::this_state ts, sol::table self, std::string key, sol::object value) {
		if (key == "brushSize") {
			if (value.is<int>()) g_gui.SetBrushSize(value.as<int>());
		}
		else if (key == "brushShape") {
			if (value.is<std::string>()) {
				std::string s = value.as<std::string>();
				if (s == "circle") g_gui.SetBrushShape(BRUSHSHAPE_CIRCLE);
				else if (s == "square") g_gui.SetBrushShape(BRUSHSHAPE_SQUARE);
			}
		}
		else if (key == "brushVariation") {
			if (value.is<int>()) g_gui.SetBrushVariation(value.as<int>());
		}
		else if (key == "spawnTime") {
			if (value.is<int>()) g_gui.SetSpawnTime(value.as<int>());
		}
	};
	...
	mapView["registerShow"] = [](sol::variadic_args va) -> bool {
		...
		if (va.size() == 2 && va[0].is<std::string>() && va[1].is<std::string>()) {
			...
			if (va[0].is<sol::table>()) {
				if (va[1].is<std::string>()) label = va[1].as<std::string>();
				if (va[2].is<std::string>()) overlayId = va[2].as<std::string>();
	...
	lua.new_usertype<Editor>("Editor",
		...
		"canUndo", [](Editor* editor) -> bool {
			return editor && editor->actionQueue && editor->actionQueue->canUndo();
		},
		"canRedo", [](Editor* editor) -> bool {
			return editor && editor->actionQueue && editor->actionQueue->canRedo();
		},
