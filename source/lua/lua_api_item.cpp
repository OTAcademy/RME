...
void registerItem(sol::state& lua) {
	...
	lua.new_usertype<Item>("Item",
		...
		"subtype", sol::property(
			[](const Item& item) -> int { return item.getSubtype(); },
			[](Item& item, int subtype) { item.setSubtype(static_cast<uint16_t>(subtype)); }
		),
		"actionId", sol::property(
			[](const Item& item) -> int { return item.getActionID(); },
			[](Item& item, int aid) { item.setActionID(static_cast<uint16_t>(aid)); }
		),
		"uniqueId", sol::property(
			[](const Item& item) -> int { return item.getUniqueID(); },
			[](Item& item, int uid) { item.setUniqueID(static_cast<uint16_t>(uid)); }
		),
		"tier", sol::property(
			[](const Item& item) -> int { return item.getTier(); },
			[](Item& item, int tier) { item.setTier(static_cast<uint16_t>(tier)); }
		),
		...
		sol::meta_function::to_string, [](const Item& item) {
			return "Item(id=" + std::to_string(item.getID()) +
				   ", name=\"" + item.getName() + "\")";
		}
	);
