...
void LiveClient::tryConnect(boost::asio::ip::tcp::resolver::results_type endpoints) {
	if (stopped) {
		return;
	}
	auto begin = endpoints.begin();
	if (begin == endpoints.end()) {
		return;
	}

	logMessage("Joining server " + begin->host_name() + ":" + begin->service_name() + "...");

	boost::asio::async_connect(*socket, endpoints, [this](const boost::system::error_code& error, const boost::asio::ip::tcp::endpoint&) -> void {
		if (error) {
