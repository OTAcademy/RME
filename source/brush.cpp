...
FlagBrush::FlagBrush(uint32_t _flag) : flag(_flag), zoneId(0)
{
	////
}
...
std::string FlagBrush::getName() const
{
	switch(flag) {
		...
		case TILESTATE_ZONE_BRUSH: return "Zone brush (0x10)";
		default:
			break;
	}
...
int FlagBrush::getLookID() const
{
	switch(flag) {
		...
		case TILESTATE_ZONE_BRUSH: return EDITOR_SPRITE_ZONE_TOOL;
		default:
			break;
	}
	return 0;
...
void FlagBrush::undraw(BaseMap* map, Tile* tile)
{
	...
	{
		if (zoneId == 0 || tile->getZoneId() == zoneId)
		{
			tile->setZoneId(0);
